    ...
    <section className="w-full bg-white py-12 sm:py-16 lg:py-20 border-b border-gray-100 flex flex-col items-center">
      ...
      <div className="lg:hidden flex flex-col items-start w-[calc(100%-48px)] max-w-[420px] mx-auto py-4 relative select-none">
        {PAYMENT_MILESTONES.map((milestone, idx) => {
          const isLast = idx === PAYMENT_MILESTONES.length - 1;
          return (
            <div key={idx} className="flex items-center gap-5 w-full relative mb-10 last:mb-0">
              
              {/* Connector Dash Line under Circle */}
              {!isLast && (
                <div className="absolute left-[31px] top-[64px] h-[40px] w-0 border-l-2 border-dashed border-[#222] flex items-center justify-center pointer-events-none z-0">
                  {/* Down arrowhead centered on the line */}
                  <div className="absolute top-1/2 -translate-y-1/2 w-0 h-0 border-t-[6px] border-x-[4px] border-t-[#222] border-x-transparent" />
                </div>
              )}

              {/* Circle Icon - crisp black */}
              <div className="w-[64px] h-[64px] bg-white border border-[#222] rounded-full flex items-center justify-center shrink-0 relative z-10">
                <Image
                  src={milestone.icon}
                  alt={milestone.title}
                  width={30}
              ...
              <div className="flex flex-col text-left">
                <span
                  className="text-[15px] sm:text-[16px] font-bold text-[#111] leading-snug mb-0.5"
        ...
        <div className="min-w-[1040px] flex justify-center py-2">
          <div className="w-[1000px] h-[160px] relative">
            ...
            <svg
              ...
              fill="none"
            >
              <path d="M 138 38 L 254 38" stroke="#222" strokeWidth="1.3" strokeDasharray="3 3" />
              <path d="M 254 33 L 262 38 L 254 43" stroke="#222" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
              <path d="M 338 38 L 454 38" stroke="#222" strokeWidth="1.3" strokeDasharray="3 3" />
              <path d="M 454 33 L 462 38 L 454 43" stroke="#222" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
              <path d="M 538 38 L 654 38" stroke="#222" strokeWidth="1.3" strokeDasharray="3 3" />
              <path d="M 654 33 L 662 38 L 654 43" stroke="#222" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
              <path d="M 738 38 L 854 38" stroke="#222" strokeWidth="1.3" strokeDasharray="3 3" />
              <path d="M 854 33 L 862 38 L 854 43" stroke="#222" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
            ...
            <div className="absolute top-0 left-0 w-[1000px] flex justify-between">
              {PAYMENT_MILESTONES.map((milestone, idx) => (
                <div key={idx} className="w-[200px] flex flex-col items-center text-center select-none">
                  ...
                  <span
                    className="text-[15px] lg:text-[16px] font-bold text-[#111] leading-snug mb-1 max-w-[180px]"
                    style={{ fontFamily: "CircularStd-Bold, sans-serif" }}
                  >
                    {milestone.title}
                  </span>

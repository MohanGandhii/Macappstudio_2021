"use client";

import React from "react";
import Image from "next/image";
import { useModal } from "@/context/ModalContext";

export default function DreamCTASection() {
  const { openProposalModal } = useModal();
  return (
    <section className="w-full py-12 lg:py-16 px-4 md:px-8 bg-white">
      <div className="max-w-[1380px] mx-auto w-full relative rounded-[20px] bg-gradient-to-r from-[#0F55FA] via-[#0C4EE0] to-[#0A3CB5] overflow-hidden shadow-[0_10px_35px_rgba(12,84,236,0.12)] min-h-[120px] lg:min-h-[140px] flex items-center">

        {/* ── BACKGROUND ILLUSTRATIONS (ABSOLUTE POSITIONED) ── */}

        {/* Left: Rocket Illustration */}
        <div className="hidden lg:block absolute bottom-[-5%] left-[-2%] w-[180px] xl:w-[240px] h-[120%] select-none pointer-events-none z-10">
          <Image
            src="/assets/images/resources/15/image 1884 (4).png"
            alt="Rocket illustration"
            fill
            className="object-contain object-bottom object-left"
            unoptimized
          />
        </div>

        {/* Right: Team Members Illustration */}
        <div className="hidden lg:block absolute bottom-0 right-[-1%] w-[260px] xl:w-[320px] h-[105%] select-none pointer-events-none z-10">
          <Image
            src="/assets/images/resources/15/IMG_20260611_000030.png"
            alt="Team members illustration"
            fill
            className="object-contain object-bottom object-right"
            unoptimized
          />
        </div>

        {/* ── CONTENT CONTAINER ── */}
        <div className="w-full relative z-20 py-8 px-5 md:px-8 lg:pl-[160px] xl:pl-[220px] lg:pr-[260px] xl:pr-[300px] flex flex-col xl:flex-row xl:items-center justify-start gap-8 xl:gap-12">

          {/* Left Text Block */}
          <div className="flex flex-col text-left shrink-0">
            <h3
              className="text-white font-extrabold text-[22px] md:text-[25px] xl:text-[28px] tracking-tight leading-tight whitespace-normal xl:whitespace-nowrap"
              style={{ fontFamily: "Outfit, sans-serif" }}
            >
              Your Dream Is Closer Than You Think
            </h3>
            <p className="text-white/90 text-[14px] xl:text-[15px] font-medium mt-1.5">
              Let's build the next big thing together.
            </p>

            {/* 4 Bullet Points (Horizontal Flex Row) */}
            <div className="flex flex-wrap xl:flex-nowrap items-start gap-x-4 xl:gap-x-5 gap-y-4 mt-6 pt-1">
              {/* Item 1 */}
              <div className="flex items-start gap-1.5 shrink-0">
                <svg className="w-[18px] h-[18px] text-white/90 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span className="text-white/80 text-[11px] xl:text-[12px] font-medium leading-[1.3] whitespace-nowrap">
                  Free 30-min <br /> strategy call
                </span>
              </div>

              {/* Item 2 */}
              <div className="flex items-start gap-1.5 shrink-0">
                <svg className="w-[18px] h-[18px] text-white/90 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                </svg>
                <span className="text-white/80 text-[11px] xl:text-[12px] font-medium leading-[1.3] whitespace-nowrap">
                  No obligation <br /> whatsoever
                </span>
              </div>

              {/* Item 3 */}
              <div className="flex items-start gap-1.5 shrink-0">
                <svg className="w-[18px] h-[18px] text-white/90 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                </svg>
                <span className="text-white/80 text-[11px] xl:text-[12px] font-medium leading-[1.3] whitespace-nowrap">
                  Actionable insights <br /> for your idea
                </span>
              </div>

              {/* Item 4 */}
              <div className="flex items-start gap-1.5 shrink-0">
                <svg className="w-[18px] h-[18px] text-white/90 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138z" />
                </svg>
                <span className="text-white/80 text-[11px] xl:text-[12px] font-medium leading-[1.3] whitespace-nowrap">
                  Expert guidance <br /> given product ninjas
                </span>
              </div>
            </div>
          </div>

          {/* Right Action Buttons */}
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-start gap-3 xl:gap-4 shrink-0 mt-6 xl:mt-0 w-full xl:w-auto">
            {/* Button 1 */}
            <button
              onClick={openProposalModal}
              className="w-full sm:w-auto bg-white hover:bg-white/95 text-[#0C54EC] text-left py-3 px-4 xl:px-5 rounded-[12px] shadow-[0_4px_15px_rgba(0,0,0,0.05)] transition-all hover:scale-[1.02] flex flex-col justify-center min-w-0 sm:min-w-[170px] xl:min-w-[190px] min-h-[64px] cursor-pointer"
            >
              <span className="font-bold text-[12px] xl:text-[13px] tracking-tight">
                Book Free Discovery Call
              </span>
              <span className="text-[#0C54EC]/75 text-[9.5px] xl:text-[10px] font-semibold mt-0.5">
                Free 30-min strategy session
              </span>
            </button>

            {/* Button 2 */}
            <button className="invisible pointer-events-none w-full sm:w-auto bg-white hover:bg-white/95 text-[#0C54EC] text-left py-3 px-4 xl:px-5 rounded-[12px] shadow-[0_4px_15px_rgba(0,0,0,0.05)] transition-all hover:scale-[1.02] flex flex-col justify-center min-w-0 sm:min-w-[170px] xl:min-w-[190px] min-h-[64px] cursor-pointer">
              <span className="font-bold text-[12px] xl:text-[13px] tracking-tight">
                Build My Dream Team
              </span>
              <span className="text-[#0C54EC]/75 text-[9.5px] xl:text-[10px] font-semibold mt-0.5">
                Start building in 1-2 weeks
              </span>
            </button>
          </div>
        </div>
      </div>
    </section>
  );
}
